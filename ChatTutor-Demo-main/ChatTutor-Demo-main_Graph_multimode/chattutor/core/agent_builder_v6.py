...
from chattutor.core.tools_v2 import search_tool_v2, kg_tool_v2
...
tools_list = [search_tool_v2]
if kg_tool_v2:
    tools_list.append(kg_tool_v2)
...
def _inject_profile(prompt_str: str, state: AgentState) -> str:
    ...
    if user_type == "primary_school":
        tone_guidance = "\n\n[语气指导：针对小学生]\n- 使用简单易懂的语言，避免复杂术语\n- 多用比喻和生动例子\n- 语气亲切友好，充满鼓励\n- 句子简短，结构清晰"
    elif user_type == "middle_school":
        tone_guidance = "\n\n[语气指导：针对中学生]\n- 语言清晰直接，适当使用专业术语但需要解释\n- 结合生活实际例子\n- 鼓励思考，培养逻辑思维\n- 保持专业但友好的态度"
    elif user_type == "university":
        tone_guidance = "\n\n[语气指导：针对大学生]\n- 使用专业术语，可以直接引用概念\n- 注重理论深度和逻辑严谨性\n- 鼓励批判性思考和自主探究\n- 保持学术性但不过于刻板"
    elif user_type == "adult":
        tone_guidance = "\n\n[语气指导：针对成人学习者]\n- 注重实用性和应用场景\n- 语言直接高效，避免冗长\n- 尊重学习者的经验和背景\n- 强调与实际工作的关联"

    combined_prompt = prompt_str
    if tone_guidance:
        combined_prompt += tone_guidance
    if summary:
        combined_prompt += "\n\n[学习档案]\n" + summary

...
调用链（后续如何被使用）：
- chattutor/core/agent_builder_v4.py
...
def load_profile(learner_id: str) -> Dict[str, Any]:
    """
    加载学习画像；不存在则返回空结构。
    被 agent_builder_v4 的 _inject_profile(...) 调用。
    """
...
def save_profile(profile: Dict[str, Any]) -> str:
    """
    保存学习画像（覆盖写）。
    被 agent_builder_v4 的 aggregator_node(...) 在每轮对话结束后调用。
    """
...
def upsert_cards(profile: Dict[str, Any], new_cards: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    合并并去重卡片。
    被 agent_builder_v4 的 aggregator_node(...) 调用。
    """
    if not new_cards:
        return profile
...
def set_user_type(profile: Dict[str, Any], user_type: str) -> Dict[str, Any]:
    """
    设置用户类型（小学生、中学生、大学生等）。
    """
    valid_types = ["primary_school", "middle_school", "university", "adult", None]
    if user_type not in valid_types:
        raise ValueError(f"无效的用户类型。有效选项: {valid_types}")

    profile["user_type"] = user_type
    return profile


def get_user_type(profile: Dict[str, Any]) -> str:
    """
    获取用户类型。
    """
...
def profile_summary(profile: Dict[str, Any], max_items: int = 6) -> str:
    ...
    if user_type:
        ...
        display_type = type_map.get(user_type, user_type)
        lines.append(f"- 用户类型: {display_type}")

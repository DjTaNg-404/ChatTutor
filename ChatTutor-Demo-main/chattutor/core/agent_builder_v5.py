...
# 仅保留最近 N 条消息到持久化日志（避免长期存储完整对话）
PERSIST_MESSAGES_LIMIT = 20
...
def _get_learner_id(state: AgentState) -> str:
    return state.get("learner_id") or state.get("session_id") or "anonymous"


def _inject_profile(prompt_str: str, state: AgentState) -> str:
    learner_id = _get_learner_id(state)
    profile = learning_profile.load_profile(learner_id)
...
def aggregator_node(state: AgentState) -> Dict[str, Any]:
    ...
    try:
        ...
        if cards:
            ...
            learning_profile.save_profile(profile)
    except Exception:
        pass

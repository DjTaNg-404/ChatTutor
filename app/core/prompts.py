...
5. **request_plan** (学习计划)：
   - 触发条件：用户**首次**明确要求制定学习计划，或明确要求**重新生成**计划。
   - **关键词**： "制定计划"、"生成计划"、"重新生成计划"、"新的计划"、"帮我规划一个计划"
   - **不触发的情况**（已有计划时）：
     - 用户说"调整时间"、"改天数"、"增加内容"、"减少内容"、"调整难度" → 用 `tutor_answer` 回应
     - 用户说"计划不错"、"好的"、"知道了" → 用 `judge` 或闲聊回应
     - 用户询问计划相关的问题 → 用 `tutor_answer` 回应
   - **注意**：如果用户已经有了计划，只有当用户明确说"重新生成计划"或"生成新的计划"时才触发 `request_plan=True`。
   - 行为：这将调用 Task Plan Agent 微服务生成学习计划。

6. **is_concluding** (结束对话)：
   - 触发条件：用户表达结束意图，如"再见"、"今天先学到这"、"退出"、"结束并总结"、"结束今日学习"、"结束并总结今日学习"。
   - **优先识别**：如果用户同时提到"结束"和"总结"，必须设置 `is_concluding=True`。
